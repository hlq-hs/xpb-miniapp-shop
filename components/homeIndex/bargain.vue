		...
		<view class="group" :style="[...boxStyle]">
			<view class="group-top acea-row row-middle row-between" :style="[bgImgStyle]">
				<view class="group-top-left acea-row">
					...
					<view class="num" :style="[titleColor]">限时活动</view>
				</view>
					...
					<view class="acea-row row-between" @click="toGroupDetail(item.id,item.product)">
						...
						<view class="group-bottom-right acea-row row-column row-between">
							<view class="right-top">
								<view class="title line2" v-if="typeShow.includes(0)" :style="[nameColor]">{{ item.title }}</view>
								<view class="pink acea-row" v-if="typeShow.includes(1)"><view class="people-box acea-row" :style="[groupTitleColor]"><view class="people" :style="[groupTitleColor]"><text class="iconfont icon-kanjiahuohua"></text><text>{{ item.sales }}</text></view></view></view>
							</view>
							<view class="right-bottom acea-row row-between">
								<view class="price">
									<view class="pinkNum" v-if="typeShow.includes(2)" :style="[priceColor]"><text class="pinkNum-title"></text><text class="pinkNum-icon">¥</text><text class="pinkNum-num semiBold">{{ item.minPrice }}</text></view>
									<view class="num otNum regular" v-if="typeShow.includes(3)" :style="[originalColor]"><text class="num-title"></text><text class="icon-num">¥{{ item.price }}</text></view>
								</view>
								<view class="btnBox" v-if="groupBtnShow"><view class="btn" :style="[...btnColor]">去砍价</view></view>
							</view>
						</view>
					</view>
				</view>
			...
			<view class="group-bottom two acea-row row-between grid-list" v-if="listStyle == 1" :style="[...boxBgStyle]">
				<view v-for="(item, index) in groupProductList" :key="index" @click="toGroupDetail(item.id)">
					...
					<view class="two-item"><view class="title acea-row"><text class="line1" v-if="typeShow.includes(0)" :style="[nameColor]">{{ item.title }}</text></view><view class="two-item-bottom acea-row row-between"><view class="price"><view class="pinkNum" v-if="typeShow.includes(2)" :style="[priceColor]"><text class="num-icon">¥</text><text class="num semiBold">{{ item.minPrice }}</text></view><view class="otNum regular" v-if="typeShow.includes(3)" :style="[originalColor]">¥{{ item.price }}</view></view><view class="btnBox" v-if="groupBtnShow"><view class="btn" :style="[...btnColor]">去砍价</view></view></view></view>
				</view>
			</view>
			<view class="group-bottom three acea-row grid-three" v-if="listStyle == 2" :style="[...boxBgStyle]">
				<view v-for="(item, index) in groupProductList" :key="index" class="three-box" @click="toGroupDetail(item.id)">
					<view class="group-bottom-left"><view class="img acea-row row-center row-middle three-img"><easy-loadimage class="loadimage" width="100%" height="100%" :image-src="item.image" :radius="dataConfig.contentStyle.val"></easy-loadimage></view></view>
					<view class="two-item"><view class="title acea-row"><text class="line1" v-if="typeShow.includes(0)" :style="[nameColor]">{{ item.title }}</text></view><view class="two-item-bottom"><view class="price"><view class="pinkNum line1" v-if="typeShow.includes(2)" :style="[priceColor]"><text class="num-icon">¥</text><text :style="[{'font-size':item.minPrice.length>6?'30rpx':'36rpx'}]" class="num semiBold">{{ item.minPrice }}</text></view><view class="otNum regular" v-if="typeShow.includes(3)" :style="[originalColor]">¥{{ item.price }}</view></view></view></view>
				</view>
			</view>
			<view class="group-bottom four acea-row" v-if="listStyle == 3" :style="[...boxBgStyle]">
				<scroll-view scroll-x="true" class="scroll_view">
					<view v-for="(item, index) in groupProductList" :key="index" class="four-item" :style="[fourStyle]" @click="toGroupDetail(item.id)">
						<view class="group-bottom-left"><view class="img acea-row row-center row-middle four-img"><easy-loadimage :image-src="item.image" width="214rpx" height="214rpx" :radius="dataConfig.contentStyle.val"></easy-loadimage></view></view>
						<view class="two-item"><view class="title acea-row" v-if="typeShow.includes(0)" :style="[nameColor]"><text class="line1">{{ item.title }}</text></view><view class="two-item-bottom"><view class="price"><view class="people-size-box" v-if="typeShow.includes(1)" :style="[...peopleStyle]"><view class="people-size">{{ item.sales }}人参与</view></view><view class="pinkNum" v-if="typeShow.includes(2)" :style="[priceColor]"><text class="num-icon">¥</text><text class="num semiBold">{{ item.minPrice }}</text></view></view></view></view>
					</view>
				</scroll-view>
			</view>
