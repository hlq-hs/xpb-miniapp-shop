		...
		<view :style="[...boxStyle]">
				...
				<view class="scroll_box">
					<scroll-view scroll-x="true" class="scroll_view acea-row">
						<view class="no-warp acea-row row-middle">
							<view class="couponBg acea-row row-middle style1" v-for="(item, index) in couponList" :key="index" :style="[...contentConfig]">
								<view class="left">
									<view :style="[...priceColorStyle]"><text class="price-icon">￥</text><text class="price" :class="item.money.length>6?'sizePrice':''">{{item.money}}</text></view>
									<view class="title" :class="item.minPrice.length>6?'sizeTitle':''">满{{ item.minPrice }}可用</view>
								</view>
								<view class="right" @click="getCoupon(item.isUse,item.id)">{{item.isUse ? '已领取' : '领取'}}</view>
							</view>
						</view>
					</scroll-view>
				</view>
				...
				<view class="coupon1 acea-row row-middle">
					<scroll-view scroll-x="true" class="scroll_view">
						<view class="list acea-row row-middle">
							<view class="item" v-for="(item, index) in couponList" :key="index" :style="[...contentConfig]">
								<view class="money" :style="[...priceColorStyle]">
									<view :class="item.money.length>=6?'sizePrice-two':''"><text class="lable" :class="item.money.length>=6?'sizeLable-two':''">￥</text>{{item.money}}</view>
									<view class="tips">满{{item.minPrice}}可用</view>
								</view>
								<view class="sill" :style="[...btnColorStyle]" @click="getCoupon(item.isUse,item.id)">{{item.isUse ? '已领取' : '立即领取'}}</view>
			...
			<template v-if="listStyle == 2">
				<view class="scroll_box">
					<scroll-view scroll-x="true" class="scroll_view acea-row">
						<view class="no-warp acea-row row-middle">
							<view class="couponBg acea-row row-middle couponBg-new" v-for="(item, index) in couponList" :key="index" :style="[...contentConfig]">
								<view class="left">
									<view :style="[...priceColorStyle]"><text class="price-icon">￥</text><text class="price" :class="item.money.length>6?'sizePrice':''">{{item.money}}</text></view>
									<view class="title" :class="item.minPrice.length>6?'sizeTitle':''">满{{ item.minPrice }}可用</view>
								</view>
								<view class="right" @click="getCoupon(item.isUse,item.id)" v-if="!item.isUse">立即领取</view>
								<view class="right" @click="getCoupon(item.isUse,item.id)" v-else>已领取</view>
							</view>
						</view>
					</scroll-view>
				</view>
			...
			<template v-if="listStyle == 3">
				<view class="coupon5 acea-row row-middle" :style="[...boxBg]">
					<scroll-view scroll-x="true" style="white-space: nowrap; display: flex">
						<view class="list acea-row row-middle">
							<view class="item acea-row row-middle" v-for="(item, index) in couponList" :key="index" :style="[...contentConfig]">
								<view class="left">
									<view :class="item.money.length>=6?'sizePrice-four':''" :style="[...priceColorStyle]" class="money"><text class="label">￥</text>{{item.money}}</view>
									<view class="tips">满{{item.minPrice}}可用</view>
								</view>
								<view class="right acea-row row-center">
									<view class="rightCon" @click="getCoupon(item.isUse,item.id)">{{item.isUse ? '已领取' : '领取'}}</view>
								</view>
